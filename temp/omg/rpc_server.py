...
def start_batch(params: dict) -> dict:
    ...
    # 5. Pass keyboard values to resolver if provided
    kb_values = params.get("keyboard_values", {})
    if kb_values:
        resolver.set_keyboard_values(kb_values)
        sys.stderr.write(f"DEBUG: keyboard_values set: {list(kb_values.keys())}\n")
        sys.stderr.flush()
